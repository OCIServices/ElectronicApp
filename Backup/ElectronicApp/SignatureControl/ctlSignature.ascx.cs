...
namespace WebSignatureCapture
{
	...
	public partial class SignatureControl : UserControl
	{
		...
		/// </summary>
		private string _noSignMsg = "No signature found Please Sign The Document!";
		...
		{
            string frmCode = @"<iframe id='frmSign' name='frmSign' MarginHeight='0' MarginWidth='0' PaddingWidth scrolling='no' src='" + Page.ResolveUrl(SignatureCodePath + "Sign.aspx") + SignURL + "' frameborder='0' style='width:" + Convert.ToString(SignWidth) + "px;height:" + Convert.ToString(SignHeight) + "px;border:1px solid black;'></iframe>";
		...
		{
            string clientScript = "<script language='javascript'>function SaveSignature(){window.frames.frmSign.save();}	function ClearSignature(){document.getElementById('frmSign').src = '" + Page.ResolveUrl(SignatureCodePath + "Sign.aspx") + SignURL + "';} </script>";
